...
> ・**.NET 10** で公開・運用するためのリファレンス実装です。
> 本番利用は想定していません。

> 🌐 English README: **[README.md](README.md)**

SaaS Offer フルフィルメントのサンプル：購入者向けの **SSO ランディングページ**
（Resolve → 明示確認のうえ Activate）、**接続 Webhook**、**権威ある購読状態ストア**、
**最小限のパブリッシャー管理画面** — これらを合わせた **フルフィルメント層**
（購読の状態を Microsoft 側と同期させる、パブリッシャー側の実装）です。公式の
