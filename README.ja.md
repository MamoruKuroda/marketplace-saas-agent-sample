...
> **Tier-1（定額 / flat-rate）**・**.NET 10** で公開・運用するための、エージェント対応リファレンス実装です。
> 本番利用は想定していません。
...
**最小限のパブリッシャー管理画面** を備え、その手前に言語非依存の **ツール境界** を置くことで、
フルフィルメント層を書き換えずに後から LLM/エージェント層を追加できます。公式の
...
| `tests/SaaSAgentSample.Tests` | ユニット＋統合（合成 L2）テスト |
...
- 状態ストア用のデータベース（ホストのアーキテクチャに応じて選択）：
  ...
  - **Windows x64（Docker なし）**：SQL Server **LocalDB**。フル SQL Server と同じ接続文字列の
    切り替えで動作します。
- 合成 L2 ウォークスルー用の Fulfillment API Emulator — [docs/l2-demo.md](docs/l2-demo.md) 参照。
