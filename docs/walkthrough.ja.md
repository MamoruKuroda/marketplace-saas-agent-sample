...
<!-- GitHub の Mermaid は日本語ラベルを見切れさせるため、PNG を事前生成して埋め込み。ソース: images/ja-buyer-journey.mmd -->
![購入者の体験：5ステップの旅](images/ja-buyer-journey.png)
...
![パブリッシャーの旅：公開までの6フェーズ](images/ja-publisher-journey.png)
...
本サンプルでは集約が**遷移を保護**（不正な遷移は拒否）し、状態が唯一の正本です — アプリがそれを
捏造することはありません。
...
<!-- GitHub の Mermaid は日本語ラベルを見切れさせるため、PNG を事前生成して埋め込み。ソース: images/ja-call-direction.mmd -->
![呼び出しの向き（Webhook だけ逆方向）](images/ja-call-direction.png)
