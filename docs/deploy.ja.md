...
> データベース権限付与）を自動化します。[クラウドにデモをデプロイ](../README.ja.md#クラウドにデモをデプロイazd)
> を参照。以下は同じことを手作業で行う手順です。各コマンドを自分で実行したい・各リソースを理解したいときに
> どうぞ。（**ローカル**で動かしたい場合は [README](../README.ja.md) を参照 — そちらは SQLite を使い Azure 不要です。）
