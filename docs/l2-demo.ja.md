...
本サンプルは、Microsoft 商用マーケットプレースの SaaS Fulfillment API を契約として用い、トークン不要の
エミュレーターを Microsoft の代役に使うことで、**実購入なしに**フルフィルメントの配管をエンドツーエンドで
実証します。ここでの **「L2」** は統合レベルの実証を指します：アプリが実 HTTP 上でフルフィルメント API
（ユニットレベルのモックではない）と通信し、接続 Webhook に反応して、購読ライフサイクル全体
（Resolve → Activate → Webhook → 状態）を駆動します。**「合成（Synthetic）」** とは、エミュレーター
（または in-repo の HTTP スタブ）が実マーケットプレース購入を置き換えることを意味し、実購入者アカウントや
実マーケットプレース購読は不要です。
